...
export default class IMap {
    ...
    async getNode (data) {
        ...
        return await this.db.getNode(this.constructor, id, data);
    }
    ...
    serialize () {
        ...
        return this.db.encode({
            id: this.id,
            keys: this.keys,
            size: this._size,
            value: this.value// this.db.encode(this.value)
        });
    }
    ...
    async load () {
        const loaded = await this.localLoad();

        if (!loaded) {
            return await this.db.remote?.pullNode(this);
        }
    ...
    async localLoad () {
        if (!this.loaded) {
            return await this.db.loadNode(this);
        }
    ...
    async toJS () {
        const values = await this._toArray();
        const r = {};
