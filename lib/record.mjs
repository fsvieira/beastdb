...
export default class Record {
    ...
    setData (data) {
        data.__version = data.__version || 1;
        const hash = this.hashData(data);
        this._data = Object.freeze({
            __hashHistory: [hash], // Object.freeze([hash]),
            __version: 1,
            ...data,
    ...
    async updateHash (data) {
        ...
        if (this.lastHash(data) !== hash) {
            data.__hashHistory.unshift(hash);
        }
    ...
    async canUpdate (newData) {
        ...
        /*
        const curr = await currentHistoryData.toArray();
        const newh = await newHistoryData.toArray();
        console.log("--->", curr, newh);
        */

        const currentHash = this.lastHash; // await currentHistoryData.getLastValue();
        // const length = await newHistoryData.length;

        // for (let i=length-1; i>= 0; i--) {
        for (let i=0; i<newHistoryData.length; i++) {
            const hash = newHistoryData[i]; // await newHistoryData.get(i);
            if (hash === currentHash) {
    ...
    async _update (data) {
        ...
        */
        ...
        if (await this.canUpdate(newData)) {
            ...
            // 1. merge hash histories, 
            const oldHashes = this._data.__hashHistory.filter(
                h => !newData.__hashHistory.includes(h)
            );

            newData.__hashHistory = newData.__hashHistory.concat(
                oldHashes
            );

            newData.__hashHistory.length = Math.min(newData.__hashHistory.length, 100);
    ...
    async save (oldData) {
        ...
        if (!oldData) {
            ...
            newData = this.updateHash(newData);
            // const hash = await this.hashData(newData);
