...
import {encode, decode} from './types.mjs';
...
export default class Storage {
    ...
    /*
        Data keys
    */
    hashValues = (values, fields=Object.keys(values)) => this.hashFn(fields.sort().map(field => {
        let v = values[field];
    ...
    */
    ...
    async saveRecord (name, schema, record, oldData) {
        ...
        // hash data,
        // const hashHistory = (await this.historyHashData(data))[Record.hashHistoryLabel];
        ...
        for (let indexID in schema.indexes) {
            ...
            const key = this.labelIndexValueId(name, fields, originalData, id);
        ...
        // record._data[Record.hashHistoryLabel] = hashHistory;
    }

    async getSavedRecord (table, id) {
        ...
        if (!record) {
            ...
            if (data) {
                record = new Record(table, id, data, true, false);
                this.data.set(key, record);
            }
        }
    ...
    */
    ...
    async getNode (T, id, newData) {
        const key = this.labelCollection(T.name, id);
        let n = this.data.get(key);

        if (!n) {
            const data = await this.get(key) || newData;
    ...
    async loadNode (node) {
        ...
        if (data) {
            ...
            const decodedData = await this.decode(data);
            node.keys = decodedData.keys;
            node.value = decodedData.value;
            node.size = decodedData.size;
            /*
    ...
    // this is used to force a node save,
    // nodes are normally saved when they are in a record, 
    // but if they are sent from network it means may already be in a record or they 
    // eventually will be on a record. 
    async saveNode (nodeJSON) {
        console.log(nodeJSON);
        const {node: {_type: type, data: id}, data: encodedData } = nodeJSON;
        const key = this.labelCollection(type, id);

        const node = this.data.get(key);

        const data = await this.decode(encodedData);

        // if node is on cache then update it. 
        node?.setNodeData(data);

        await this.storage.put(key, encodedData);
    }
    ...
    */
    async *findByIndex (table, obj) {
        const id = obj[table.schema.key.id];
        ...
        if (id) {
            ...
            for (let field in obj) {
                const value = await record.data[field];
            ...
            if (table.schema.indexes[indexID]) {
                const index = this.labelIndexValue(table.name, fields, obj);
