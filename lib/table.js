...
class Table {
    ...
    async find (id) {
        ...
        if (!local) {
            local = await this.db.remote?.pullRecord(this.name, id);
        }
    ...
    /*
    async getRemoteChanges (id, hash) {
        const srcRecord = await this.find(id);
        ...
        if (hash) {
            ...
            switch (action) {
                ...
                case 'divergent':
                    // we need to send and pull
                    this.db.pushPull.pushPull(id, await s.__hashHistory.getLastValue());
                    break;

                case 'outdated':
                    // we need to pull from,
                    this.db.pushPull.pushPull(id, await s.__hashHistory.getLastValue());
                    // there is no changes to send,
                    return null; 
            }
        }

        return this.db.encode(s);
    }*/
    ...
    async applyRecordChanges (changes) {
        ...
        for (let i=0; i<changes.length; i++) {
            ...
            const snapshot = await this.db.decode(encoded);
