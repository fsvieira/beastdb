...
class Table {
    ...
    async _fetch (src, id, localRecord) {
        const srcRecord = await src.tables[this.name].find(id);
        const s = await srcRecord.snapshot();
        const encoded = s.serialize();

        // TODO we need to send a record snapshot??
        throw 'Send record snapshot';
        const record = await this.db.decode(encoded);
